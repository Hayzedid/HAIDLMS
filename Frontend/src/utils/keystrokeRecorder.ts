...
export interface KeystrokeEvent {
  timestampMs: number; // Milliseconds since session start
  eventType: "keydown" | "keyup" | "paste" | "cut" | "copy";
  keyCode?: number;
  keyName: string;
  ...
  charInserted?: string;
  charsDeleted?: string;
  // Aliases for common property access patterns
  id?: string;
  timestamp?: number;
  key?: string;
  cursorPos?: number;
...
export class KeystrokeRecorder {
  ...
  private onBatchReady?: (events: KeystrokeEvent[]) => void;
  private batchSize: number = 50; // Send to server every 50 events
  private recordAllKeys: boolean = false; // Only record typing events by default

  constructor(
    sessionId: string,
    userId: string,
    recordAllKeys: boolean = false,
    onBatchReady?: (events: KeystrokeEvent[]) => void,
  ) {
   ...
   */
  startRecording(editorElement: HTMLElement): () => void {
    ...
    const keydownHandler = (e: KeyboardEvent) => {
      ...
      let charInserted: string | undefined;
      let charsDeleted: string | undefined;
    ...
    const clipboardHandler = (e: ClipboardEvent) => {
      ...
      const eventType = e.type as "paste" | "cut" | "copy";
      const selection = window.getSelection();
      const cursorPosition = selection?.anchorOffset;
   ...
   */
  private detectSuspiciousBurst(): boolean {
   ...
   * Get session summary
   */
  getSessionSummary(): Partial<KeystrokeSession> {
    const totalKeystrokes = this.events.filter((e) => !e.isSpecialKey).length;
    const totalDeletions = this.events.filter(
      (e) => e.keyName === "Backspace" || e.keyName === "Delete",
    ).length;
  ...
  async replaySession(
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (nextEvent) {
        const delay =
          (nextEvent.timestampMs - event.timestampMs) / speedMultiplier;
...
export function useKeystrokeRecorder(
  ...
  return {
    recorder,
    startRecording: (element: HTMLElement) => recorder.startRecording(element),
...
export class KeystrokePatternAnalyzer {
   ...
   */
  static detectSpeedAnomaly(
    currentSpeedWPM: number,
   ...
   */
  static calculateConsistencyScore(intervals: number[]): number {
    if (intervals.length < 10) return 100; // Not enough data
   ...
   */
  static detectAICodePattern(events: KeystrokeEvent[]): {
    isLikelyAI: boolean;
    confidence: number;
    reasons: string[];
  } {
    ...
    const pasteEvents = events.filter(
      (e) =>
        e.eventType === "paste" && e.charInserted && e.charInserted.length > 50,
    );
    ...
    const deletionRate =
      events.filter((e) => e.keyName === "Backspace" || e.keyName === "Delete")
    ...
    const hasLongPauseBeforeBurst = intervals.some((interval, i) => {
      if (interval > 30000) {
        ...
        return (
          nextIntervals.length >= 20 && nextIntervals.every((int) => int < 100)
        );
      }
      return false;
    });
    ...
    if (hasLongPauseBeforeBurst) {
      suspicionScore += 30;
      reasons.push(
        "Detected long pause followed by typing burst (possible AI query + paste)",
      );
    }
   ...
   * Detect typing burst
   */
  private static detectTypingBurst(events: KeystrokeEvent[]): boolean {
    for (let i = 0; i < events.length - 20; i++) {
      const window = events.slice(i, i + 20);
      const timeSpan =
        window[window.length - 1].timestampMs - window[0].timestampMs;
