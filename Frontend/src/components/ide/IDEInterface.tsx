...
type TabType = 'output' | 'tests' | 'submissions';
...
export default function IDEInterface({
  ...
  const [code, setCode] = useState<string>(initialCode || DEFAULT_TEMPLATES[initialLanguage]);
  ...
  const executeNormal = async () => {
    try {
      ...
      if (result.timedOut) {
        ...
        addOutputLine(
          'success',
          `✓ Completed in ${result.metrics.executionTimeMs}ms (Exit code: ${result.output.exitCode})`
        );
      } else {
        addOutputLine(
          'error',
          `✗ Process exited with code ${result.output.exitCode}`
        );
      }
      ...
      if (result.metrics.memoryUsedKB) {
        addOutputLine(
          'system',
          `📊 Memory used: ${(result.metrics.memoryUsedKB / 1024).toFixed(2)} MB`
        );
      }
    } catch (error: any) {
  ...
  const executeWithStreaming = async () => {
    try {
      ...
      const ws = ideApi.connectExecutionStream(executionId, (message: StreamMessage) => {
        switch (message.type) {
          case 'status':
          ...
          case 'complete':
            addOutputLine(
              'success',
              `✓ Completed in ${message.data.executionTimeMs}ms (Exit code: ${message.data.exitCode})`
            );
            if (message.data.memoryUsedKB) {
              addOutputLine(
                'system',
                `📊 Memory used: ${(message.data.memoryUsedKB / 1024).toFixed(2)} MB`
              );
            }
            setIsExecuting(false);
            break;
  ...
  const handleSubmit = async () => {
    ...
    const confirmed = window.confirm(
      'Are you sure you want to submit your code for grading? This will run all test cases including hidden tests.'
    );
  ...
  const handleLanguageChange = (newLanguage: Language) => {
    if (code !== DEFAULT_TEMPLATES[language] && code.trim()) {
      const confirmed = window.confirm(
        'Changing the language will reset your code. Are you sure?'
      );
      if (!confirmed) return;
    }
      ...
      <div style={{ display: 'flex', gap: '16px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ width: '45%', display: 'flex', flexDirection: 'column' }}>
              ...
              {(testResults || submissionResult) && (
                <span
                  style={{
                    ...
                    fontSize: '12px',
                    backgroundColor:
                      (submissionResult?.passed || testResults?.failed === 0) ? '#d1fae5' : '#fee2e2',
                    color:
                      (submissionResult?.passed || testResults?.failed === 0) ? '#059669' : '#dc2626',
                  }}
                >
                  {submissionResult
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {activeTab === 'output' && (
              <OutputConsole lines={outputLines} height="calc(100vh - 400px)" />
            )}
              ...
              <div>
                {submissionResult ? (
                  <TestResults
                    ...
                    totalTests={submissionResult.tests.total}
                    passedTests={submissionResult.tests.passed}
                    failedTests={submissionResult.tests.failed}
                    score={submissionResult.score}
                    executionTime={submissionResult.executionTimeMs}
                  />
                ) : testResults ? (
                  <TestResults
